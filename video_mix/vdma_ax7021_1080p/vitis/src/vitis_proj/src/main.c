...
//      return(XST_FAILURE);
    ...
    //Configure the TPG
    tpg_cfg(&tpg_inst0, 480, 720, colorFmtIn, XTPG_BKGND_SOLID_BLUE);

    //Configure the moving box of the TPG0
    tpg_box(&tpg_inst0, 25, 2);
...
void vdma_config_0(void)
{
    ...
    u32 bytePerPixels = 3;
    int offset0 = (30*1920+100)*3; // (y*w+x)*Bpp
...
void vdma_config_1(void)
{
    /* Start of VDMA Configuration */
    u32 bytePerPixels = 3;
    int offset0 = (20*1920+1000)*3;; // (y*w+x)*Bpp
//    int offset1 = 0; // (y*w+x)*Bpp
...
int main(void)
{
    ...
    {
    	 sleep(2);
    	  xil_printf("- freq: %d -\r\n", AXI_LITE_REG_mReadReg(XPAR_AXIS_PASSTHROUGH_MON_0_S00_AXI_BASEADDR, \
    	            AXI_LITE_REG_S00_AXI_SLV_REG2_OFFSET));
    	  xil_printf("- res: %dx%d -\r\n", AXI_LITE_REG_mReadReg(XPAR_AXIS_PASSTHROUGH_MON_0_S00_AXI_BASEADDR, \
    	        AXI_LITE_REG_S00_AXI_SLV_REG0_OFFSET),AXI_LITE_REG_mReadReg(XPAR_AXIS_PASSTHROUGH_MON_0_S00_AXI_BASEADDR, \
